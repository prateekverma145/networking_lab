...
void *HandleRequest(void *sockfd_ptr)
{
    int sockfd = *((int *)sockfd_ptr);
    free(sockfd_ptr); // Free the dynamically allocated memory
    char buff[MAX];
    int count_even, count_odd, count_palindrome;
...
int main()
{
    ...
    while (1)
    {
        ...
        *new_sock = connfd;
        pthread_create(&thread_id, NULL, HandleRequest, (void *)new_sock);
        pthread_detach(thread_id); // Detach thread to allow resources to be cleaned up automatically
    }
